...
function! bk#scripts#rtp_add_command(...) abort
  let l:dirs = s:rtp_args(a:000)
  let l:opt_value = join(l:dirs, ',')
  " call() because a:000 = [...]
  " bk#scripts#rtp_rm_command(a:000) -> a:000 = [[...]]
  " call(..., a:000) -> a:000 = [...]
  " ONLY BECAUSE bk#scripts#rtp_rm_command uses (...)
  " s:rtp_args uses a:args to avoid this, but we want (...) for <f-args>
  let l:rm_cmd = call('bk#scripts#rtp_rm_command', a:000)
...
function! bk#scripts#rtp_rm_command(...) abort
  let l:dirs = s:rtp_args(a:000)
  let l:rm_cmd = join(
